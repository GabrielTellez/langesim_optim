...
class Simulator(nn.Module):
    ...
    def __init__(self,
        ...
        self.register_buffer("noise_scaler", torch.tensor(noise_scaler, dtype=torch.float))
        self.device = device
        self.force = force
    ...
    def forward(self, x0: torch.tensor) -> torch.tensor:
        ...
        times = torch.arange(start=0.0, end=(self.tot_steps + 1.0) * self.dt, step=self.dt, dtype=torch.float)
        for t_idx, t in enumerate(times[:-1]):
            xnew = self.x + self.force(self.x, t) * self.dt + torch.randn(N, device=self.device) * (2.0 * self.dt * self.noise_scaler)**0.5
            if self.compute_work_heat:
                next_t = times[t_idx+1]
                self.w += self.force.potential(xnew, next_t) - self.force.potential(xnew, t)
                self.q += self.force.potential(xnew, t) - self.force.potential(self.x, t)
            self.x = xnew
...
class VariableHarmonicForce(BaseHarmonicForce):
    ...
    def __init__(self, 
        ...
        if steps is None and k is None:
            raise ValueError("Please provide initial number of `steps` or a list of initial values `k`")
        ...
        if k is None:
            if steps < 1 or not(isinstance(steps, int)):
                ...
                k_ini = torch.linspace(kappai, kappaf, steps) + 0.25 * (kappaf-kappai) * torch.randn(steps, dtype=torch.float)
                self.k = nn.parameter.Parameter(data=k_ini, 
                                               requires_grad=True)
        else:
            if steps is None or steps == len(k):
                self.k = nn.parameter.Parameter(data=torch.tensor(k, dtype=torch.float), 
                                               requires_grad=True)
            else:
                raise ValueError(f"List of initial values of the stiffness {k=} has to be equal to provided {steps=}")
        ...
        if self.continuous:
            ...
            if idx >= 0 and idx < N-1:
                ...
                if t >= N * dt and t <= self.tf:
                    k = self.k[N-1] + (self.kappaf - self.k[N-1]) * (t - N * dt) * dt**-1
...
def gaussian(x, var):
    return (2.0 * torch.tensor(np.pi) * var)**-0.5 * torch.exp(-x**2 / (2.0 * var))

def FT_pdf(pdf, kf, scale=5.0, steps=10_000, kFs=None, args=(), device=device):
    """
...
def loss_fn_k(xf, kf, ki, sim: Simulator, device=device, scale=5.0, kFsteps = 1000, x_steps=10_000):
    """
    ...
    char_P_k_teo, _ = FT_pdf(pdf=gaussian, kf=kf, scale=scale, steps=x_steps, kFs=kFs, args=(1.0/kf,), device=device)
    
    # return torch.abs((char_P_k - char_P_k_teo)**2).mean(), char_P_k, char_P_k_teo, kFs
    return torch.abs((char_P_k - char_P_k_teo)**2).mean()

def loss_fn_variance(xf: torch.tensor, kf: float, ki: float, sim: Simulator, device=device):
    """
...
def loss_fn_grad_k(ki, kf, sim: Simulator):
    ...
    if sim.force.continuous:
        ...
        ks = torch.cat((torch.tensor([ki], device=device), sim.force.k, torch.tensor([kf], device=device)))
    else:
        ks = sim.force.k
    return torch.mean(ks.diff()**2)

def loss_fn_control_k_vars(xf, kf,  ki, sim: Simulator, device=device, scale=5.0, kFsteps=1_000, x_steps=1_000, blend=1e-3):
    """Loss function that penalizes strong variations of consecutive values of kappa."""
    loss = (1.0-blend)*loss_fn_k(xf, kf, ki, sim, device=device, scale=scale, kFsteps=kFsteps, x_steps=x_steps) + blend*loss_fn_grad_k(ki, kf, sim)
    return loss
...
def loss_fn_eq_work(xf, kf,  ki, sim: Simulator, device=device, scale=5.0, kFsteps=1_000, x_steps=1_000, blend=1e-3):
    """Loss function to minimize work and distance to equilibrium"""
    loss = (1.0-blend)*loss_fn_k(xf, kf, ki, sim, device=device, scale=scale, kFsteps=kFsteps, x_steps=x_steps) + blend*loss_fn_work(xf, ki, kf, sim)
    return loss
...
def plot_test_hist(tot_sims: int, ki: float, kf: float, 
                   sim: Simulator, device=device, xrange=1.0, xpoints=200,
...
def plot_protocols(sim: Simulator, ki, kf, tf, k_comp=None, t_steps=200, sim_label="Trained", 
                   comp_label="Theoretical", title="Stiffness $\kappa$", times=None, yrange=None, 
                   grid=True, time_ticks=None, y_ticks=None, y_ticklabels=None):
    if times is None:
...
def train_loop_snapshots(epochs, 
                        sim: Simulator,
                        tot_sims,
                        ki, 
                        kf,
    ...
    """
    lossl = []
    protocols = [plot_protocols(sim, ki, kf, tf, k_comp=None, 
                                t_steps=200, sim_label=f"Trained at epoch=0", comp_label="Theoretical", 
                                title="Stiffness $\kappa$", times=times, yrange=yrange,
                               grid=grid_protocol, y_ticks=y_ticks, y_ticklabels=y_ticklabels)]
    plots = [plot_test_hist(tot_sims, ki, kf, sim, device=device, xrange=xrange, grid=grid_histo, bins=bins)]
    ...
    for snapshots in range(tot_snapshots+1):
        ...
        lossi = train_loop(epochs_to_do, 
                           ...
                           **kwargs)
        epoch += epochs_to_do
        protocol = plot_protocols(sim, ki, kf, tf, k_comp=None, t_steps=200, sim_label=f"Trained at epoch={epoch}", 
                                  comp_label="Theoretical", title="Stiffness $\kappa$", times=times, yrange=yrange,
                                 grid=grid_protocol, y_ticks=y_ticks, y_ticklabels=y_ticklabels)
        pl = plot_test_hist(tot_sims, ki, kf, sim, device=device, xrange=xrange, grid=grid_histo, bins=bins)
        lossl = lossl + lossi
        plots.append(pl)
