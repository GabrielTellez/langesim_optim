...
def test_VariableStiffnessHarmonicForcePolynomial_initialization():
    ...
    force = VariableStiffnessHarmonicForcePolynomial(kappai=kappai, kappaf=kappaf, tf=tf, coef_list=coef_list)
...
def test_VariableStiffnessHarmonicForcePolynomial_kappa_discontinuous():
    ...
    force = VariableStiffnessHarmonicForcePolynomial(kappai=kappai, kappaf=kappaf, tf=tf, coef_list=coef_list, continuous=False)
...
def test_VariableStiffnessHarmonicForcePolynomial_kappa_continuous():
    ...
    force = VariableStiffnessHarmonicForcePolynomial(kappai=kappai, kappaf=kappaf, tf=tf, coef_list=coef_list, continuous=True)

    t = torch.tensor(0.1, dtype=torch.float)
    expected_kappa = torch.tensor(kappaf)
    assert torch.isclose(force.kappa(t), expected_kappa), f"expected {expected_kappa} at t=tf, got {force.kappa(t)}"

    t = torch.tensor(0.0, dtype=torch.float)
    expected_kappa = torch.tensor(kappai)
    assert torch.isclose(force.kappa(t), expected_kappa), f"expected {expected_kappa} at t=0, got {force.kappa(t)}"

    t = torch.tensor(0.05, dtype=torch.float)
    expected_kappa = coef_list[0] + coef_list[1]*t + coef_list[2]*t**2
    expected_kappa = expected_kappa * (t/tf) * (1 - t/tf) 
    expected_kappa = expected_kappa + (kappaf - kappai) * t/tf + kappai
    assert torch.isclose(force.kappa(t), expected_kappa), f"expected {expected_kappa} at t=0.05, got {force.kappa(t)}"
...
def test_VariableStiffnessHarmonicForcePolynomial_before_after():
    ...
    force = VariableStiffnessHarmonicForcePolynomial(kappai=kappai,
                                                    kappaf=kappaf, tf=tf, coef_list=coef_list, continuous=True)

    t = torch.tensor(-0.1, dtype=torch.float)
    expected_kappa = torch.tensor(kappai)
    assert torch.isclose(force.kappa(t), expected_kappa), f"expected {expected_kappa} at t=-0.1, got {force.kappa(t)}"

    t = torch.tensor(0.2, dtype=torch.float)
    expected_kappa = torch.tensor(kappaf)
    assert torch.isclose(force.kappa(t), expected_kappa), f"expected {expected_kappa} at t=0.2, got {force.kappa(t)}"