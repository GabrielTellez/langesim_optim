...
def test_interpolator_TSP():
    ...
    def test_interpolate_function_continuous():
        ...
        assert np.isclose(
            interpolate(ti + (tf - ti) / 3, yi, yf, ti, tf, ylist), ylist[0]
        )
        assert np.isclose(
            interpolate(ti + 2 * (tf - ti) / 3, yi, yf, ti, tf, ylist), ylist[1]
        )
...
def test_interpolate_discontinuous():
    ...
    assert np.isclose(
        interpolate(ti + (tf - ti) / 3, yi, yf, ti, tf, ylist, continuous=False),
        ylist[0] + (ylist[1] - ylist[0]) / 3,
    )
    assert np.isclose(
        interpolate(ti + 2 * (tf - ti) / 3, yi, yf, ti, tf, ylist, continuous=False),
        ylist[0] + 2 * (ylist[1] - ylist[0]) / 3,
    )
